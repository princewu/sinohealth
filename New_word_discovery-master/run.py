...
def load_data(filename, stopwords):
    ...
    with open(filename, 'r', encoding='utf-8') as f:
        for line in f:
            word_list = [x for x in jieba.cut(line.strip(), cut_all=False,HMM=True) if x not in stopwords] # 对语料切词，两个参数：cut_all=：精确模式（False）/全部模式（True）； HMM=：unigram（True）/全概率（False）
...
if __name__ == "__main__":
    ...
    if os.path.exists(root_name):
        ...
        # dict_name = 'E:/HaoWu/data/train_data.txt'
        dict_name = basedir + '/data/dict.txt'
        word_freq = load_dictionary(dict_name)  # 外部词典，导入
        root = TrieNode('*', word_freq)
        save_model(root, root_name)

    # 加载新的文章
    filename = 'E:/HaoWu/data/train_data.txt'
